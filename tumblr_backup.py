...
Initialize the Tumblr backup tool
    ...
    Args:
        ...
        incremental_hours: Only fetch posts older than N hours (default: 5, set to None for full backup)
...
def fetch_all_posts(self) -> List[Dict[str, Any]]:
    ...
    Only fetches posts from 2025 onwards.
    If incremental_hours is set, only fetches posts older than N hours.
    ...
    EARLIEST_TIMESTAMP = int(datetime(2025, 1, 1, tzinfo=self.tz).timestamp())

    cutoff_timestamp = None
    if self.incremental_hours:
        cutoff_timestamp = int(time.time()) - (self.incremental_hours * 3600)
        print(f"Fetching posts older than {self.incremental_hours} hours...")
    else:
        print(f"Fetching all posts from {self.blog_identifier}...")
    ...
    while True:
        ...
        if cutoff_timestamp:
            # Only retrieve posts within the 2025+ window and older than the cutoff
            old_posts = [
                p for p in posts
                if EARLIEST_TIMESTAMP <= p.get("timestamp", 0) <= cutoff_timestamp
            ]
            all_posts.extend(old_posts)

            # If the oldest post on this page is past the cutoff, all further pages will be too
            if posts[-1].get("timestamp", 0) <= cutoff_timestamp:
                print(f"Reached cutoff boundary. Total posts fetched: {len(all_posts)}")
                break
        else:
            # No hour cutoff — still apply the 2025 floor
            eligible = [p for p in posts if p.get("timestamp", 0) >= EARLIEST_TIMESTAMP]
            all_posts.extend(eligible)

            # Stop paginating once posts go before 2025
            if posts[-1].get("timestamp", 0) < EARLIEST_TIMESTAMP:
                print(f"Reached 2025 boundary. Total posts fetched: {len(all_posts)}")
                break
...
def download_attachments(self, attachments_url: str, attachments_dir: Path, timestamp: int) -> str:
    """
    Download a attachments file and save it locally with timestamp-based filename
    """
    try:
...
def process_npf_content_blocks(
self,
        ...
        if post_type == "text":
            ...
            if video_url:
                ...
                if self.download_videos and not self.is_external_attachments(video_url, "video"):
                    video_path = self.download_attachments(video_url, attachments_dir, timestamp)
            ...
            if audio_url:
                ...
                if self.download_audio and not self.is_external_attachments(audio_url, "audio"):
                    audio_path = self.download_attachments(audio_url, attachments_dir, timestamp)
...
def save_daily_posts(self, date_key: str, posts: List[Dict[str, Any]]) -> None:
    ...
    day_dir = self.output_dir / year / month / "Tumblr"
